...
export async function initDictionary() {
  ...
  try {
    ...
    if (isBrowser) {
      ...
      // On Vercel, files might be in several places. 
      // 1. Local dev: process.cwd()/public/dictionaries/...
      // 2. Vercel build: path.join(process.cwd(), 'dictionaries') OR 'public/dictionaries'
      
      const possiblePaths = [
        path.join(process.cwd(), 'public', 'dictionaries', 'ru-petr1708-hunspell-3.1', 'ru_petr1708'),
        path.join(process.cwd(), 'dictionaries', 'ru-petr1708-hunspell-3.1', 'ru_petr1708'),
        path.join(path.dirname(fileURLToPath(import.meta.url)), '..', '..', 'public', 'dictionaries', 'ru-petr1708-hunspell-3.1', 'ru_petr1708')
      ];
      ...
      for (let i = 1; i <= 29; i++) {
        ...
        if (!found) {
          console.warn(`Could not find dictionary part ${i} in any of ${possiblePaths}`);
        }
      }
    }
