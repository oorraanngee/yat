...
export default function ApiSetup() {
  ...
  const handleBirthdaySubmit = async () => {
    ...
    if (age < 5 || age > 99) {
      setError('Извините, но вамъ должно быть отъ 5 до 99 лѣтъ для полученія ключа.');
      return;
    }
  ...
  const handleSendSms = async () => {
    ...
    try {
      ...
      setError('Ошибка отправки SMS. Убѣдитесь, что номеръ ввѣденъ въ международномъ форматѣ (напр. +7...).');
      console.error(err);
    } finally {
      setLoading(false);
    }
  ...
  const handleVerifyCode = async () => {
    ...
    try {
      ...
      setError('Неверный кодъ подтвержденія.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSkipSms = async () => {
    ...
    try {
      ...
      setError('Ошибка при генераціи ключа.');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-2xl mx-auto w-full py-12">
      <div className="text-center mb-12">
        <h1 className="text-3xl uppercase tracking-[10px] text-[#f4ecd8] mb-4">Полученіе API Ключа</h1>
        ...
        <AnimatePresence mode="wait">
          ...
          {step === 1 && (
            ...
            >
              ...
              <div className="text-center">
                <h2 className="text-xl text-[#f4ecd8] mb-2 font-bold">Шагъ 1: Авторизація</h2>
                <p className="text-[#856a54] italic">Войдите черезъ Google для начала процесса.</p>
              </div>
              <button 
              ...
              >
                {loading ? <Loader2 className="animate-spin" /> : 'Войти черезъ Google'}
              </button>
            </motion.div>
          )}
          ...
          {step === 2 && (
            ...
            >
              <div className="flex items-center gap-4 text-[#f4ecd8]">
                ...
                <h2 className="text-xl font-bold">Шагъ 2: Дата рожденія</h2>
              </div>
              <p className="text-[#856a54] italic text-sm">Согласно правиламъ, мы должны убѣдиться въ вашемъ возрастѣ.</p>
              
              <div className="flex flex-col gap-2">
                <label className="text-[#f4ecd8] text-sm uppercase tracking-widest px-1">Вашъ день рожденія</label>
                <input 
                  type="date" 
                  value={birthday}
          ...
          {step === 3 && (
            ...
            >
              <div className="flex items-center gap-4 text-[#f4ecd8]">
                ...
                <h2 className="text-xl font-bold">Шагъ 3: SMS Подтвержденіе</h2>
              </div>
              
              {!confirmationResult ? (
                <>
                  <p className="text-[#856a54] italic text-sm">Введите вашъ номеръ телефона для полученія кода.</p>
                  <input 
                    type="tel" 
                  ...
                  {smsFailedAttempts >= 5 && (
                    ...
                    >
                      Пропустить подтвержденіе (техническая ошибка)
                    </button>
                  )}
                </>
              ) : (
                <>
                  <p className="text-[#856a54] italic text-sm">Кодъ отправленъ на {phoneNumber}. Введите его ниже.</p>
                  <input 
                    type="text" 
                  ...
                  <button onClick={handleVerifyCode} disabled={loading} className="cta-button">
                    {loading ? <Loader2 className="animate-spin inline mr-2" /> : 'Подтвердить кодъ'}
                  </button>
          ...
          {step === 4 && (
            ...
            >
              ...
              <div>
                <h2 className="text-2xl text-[#f4ecd8] mb-2 font-bold uppercase tracking-widest">Ключъ готовъ!</h2>
                <p className="text-[#856a54] italic">
                  Поздравляемъ! Вы успѣшно прошли верификацію и получили доступъ къ API «ЯТЬ».
                </p>
              </div>
              <button 
                ...
                className="cta-button w-full flex items-center justify-center gap-2"
              >
                Въ личный кабинетъ <ArrowRight size={18} />
              </button>
            </motion.div>
          )}
        </AnimatePresence>
