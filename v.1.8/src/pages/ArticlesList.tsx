...
export default function ArticlesList() {
  ...
  const getAllArticles = (itemsList: ManifestItem[]): ArticleMeta[] => {
    let results: ArticleMeta[] = [];
    itemsList.forEach(item => {
      if (item.type === 'article') {
        results.push(item);
      } else {
        results = [...results, ...getAllArticles(item.items)];
      }
    });
    return results;
  };
      ...
      <div className="w-full flex flex-col gap-4">
        {searchQuery ? (
          <div className="flex flex-col gap-4">
            <h3 className="text-[#856a54] uppercase tracking-widest text-xs mb-2">Результаты поиска ({filteredArticles?.length})</h3>
            {filteredArticles?.map(article => (
              ...
              >
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <h3 className="text-xl font-bold text-[#f4ecd8]">{article.title}</h3>
                </div>
