  ...
  return (
    <div className="flex flex-col w-full h-full">
      <div className="flex justify-center mb-8">
        ...
        >
          <span className="text-xs uppercase tracking-widest font-bold">
            {direction === 'toOld' ? 'На дореформенный языкъ' : 'На современный языкъ'}
          </span>
      ...
      <main className="w-full grid grid-cols-1 md:grid-cols-2 gap-6 flex-grow">
        ...
        <div className="flex flex-col gap-2 h-[450px]">
          <div className="flex justify-between text-xs uppercase tracking-widest text-[#d4af37] font-bold">
            <span>{direction === 'toOld' ? 'Современный русскій' : 'Дореформенный'}</span>
        ...
        <div className="flex flex-col gap-2 h-[450px]">
          <div className="flex justify-between text-xs uppercase tracking-widest text-[#d4af37] font-bold">
            <span>{direction === 'toOld' ? 'Дореформенный' : 'Современный русскій'}</span>
            <span className="opacity-50">РЕЗУЛЬТАТЪ</span>
          </div>
          <div className="text-area-container text-area-translated flex-grow flex flex-col">
            <textarea 
              className="flex-grow resize-none p-5 bg-transparent text-lg focus:outline-none relative z-10"
              placeholder="Результатъ появится здѣсь..."
      ...
      <div className="flex flex-col items-center gap-4 py-8">
        ...
        >
          {isTranslating ? 'Переводъ...' : direction === 'toOld' ? 'Перевести на дореформенный языкъ' : 'Вернуть на современный языкъ'}
        </button>
