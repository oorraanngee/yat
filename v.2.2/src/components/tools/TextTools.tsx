...
export const TextToHtml = () => {
    const [text, setText] = useState('Первая строка\nВторая строка\n\nТретья строка с **жирным** текстом');
    ...
    const htmlOutput = convertToHtml(text);

    return (
        <div className="flex flex-col gap-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="flex flex-col gap-2">
                    <label className="text-[10px] text-[#856a54] uppercase font-bold px-1">Введите текст:</label>
                    <textarea 
                        className="input-field h-64 w-full font-sans text-sm" 
                        value={text} 
                        onChange={e => setText(e.target.value)}
                        placeholder="Пишите здесь... Переносы строк станут <br/>, а **текст** станет жирным."
                    />
                </div>
                <div className="flex flex-col gap-2">
                    <label className="text-[10px] text-[#856a54] uppercase font-bold px-1">HTML Код:</label>
                    <textarea 
                        className="input-field h-64 w-full font-mono text-xs text-[#d4af37]" 
                        readOnly 
                        value={htmlOutput}
            ...
            <div className="p-4 bg-white text-black rounded border border-[#3d2b20] min-h-[100px]">
                <label className="text-[10px] text-gray-400 uppercase font-bold mb-2 block">Визуальный предпросмотр:</label>
