  ...
  useEffect(() => {
    ...
    fetch('/articles/manifest.json')
      ...
      .then((data: { items: ManifestItem[] }) => {
        // Recursive search through items and categories
        const findArticle = (list: ManifestItem[]): ArticleMeta | undefined => {
          for (const item of list) {
            if (item.type === 'article' && item.id === slug) {
              return item;
            } else if (item.type === 'category') {
              const found = findArticle(item.items);
        ...
        const articleMeta = findArticle(data.items || []);
  ...
  if (error) {
    return (
      <div className="text-center py-10">
        ...
        <div className="mt-4">
          <Link to="/articles" className="text-[#d4af37] hover:underline">← Вернуться к списку</Link>
        </div>
      </div>
    );
  }
